...
public class VoltDBAdHocConnector extends EsperDataConnector{
    ...
    public void dropTables() {
    	...
    	String vcdl = "DROP VIEW v_votes_by_contestant IF EXISTS;";
    	String acsdl = "DROP TABLE area_code_state IF EXISTS;";
    	String lbtdl = "DROP TABLE w_rows IF EXISTS;";
    ...
    public void initializeDatabase() {
		...
		String lbt = "create table w_rows (vote_id bigint primary key, " +
...
//                System.out.println("warn: " + warn.getMessage());
	...
	public long getLeaderboardSize() {
		try {
			ResultSet result = executeQueryWithResult("select count(*) as num_votes from w_rows");
	...
	public boolean deleteCutoff(long cutoff) {
		return executeQuery("delete from w_rows where win_id <= " + cutoff);
	}

	@Override
	public boolean insertLeaderboard(Vote v) {
		final String values = "insert into w_rows (" + v.outputColumns() + ",win_id) values (" + v.outputValues() + "," + windowId + ")";
		boolean success = executeQuery(values);
		windowId++;
		return success;
	}
	...
	public int findLowestContestant() {
		try {
			ResultSet result = executeQueryWithResult("select contestant_number, num_votes from v_votes_by_contestant order by num_votes, contestant_number desc");
	...
	public int findTopContestant() {
		try {
			ResultSet result = executeQueryWithResult("select contestant_number, num_votes from v_votes_by_contestant order by num_votes desc, contestant_number asc");
	...
	public String printAllVotes() {
		try {
			ResultSet result = executeQueryWithResult("select contestant_number, count(*) as num_votes from v_votes_by_contestant order by num_votes, contestant_number desc"); 
